...
use Botble\ACL\Tables\VendorTable;
...
class VendorController extends BaseController
{
    ...
    public function getUserProfile($id, Request $request, FormBuilder $formBuilder)
    {
        ...
        $companyForm = $formBuilder
            ->create(CompanyForm::class, ['model' => $user->company])
            ->setUrl(route('vendors.update-company', $user->id));
        ...
        $companyForm = $companyForm->renderForm();
        $passwordForm = $passwordForm->renderForm();

        return view('core/acl::users.profile.vendor-base', compact('user', 'form', 'passwordForm', 'companyForm', 'canChangeProfile'));
    }
    ...
    public function postUpdateCompany($id, UpdateCompanyProfileRequest $request, BaseHttpResponse $response)
    {
        $user = $this->userRepository->findOrFail($id);
