...
use Botble\ACL\Enums\UserCNICStatusEnum;
...
class VendorTable extends TableAbstract
{
    ...
    public function ajax()
    {
        $data = $this->table
            ...
            ->editColumn('username', function ($item) {
                ...
                return Html::link(route('vendors.profile.view', $item->id), $item->username);
            })
            ...
            ->editColumn('cnic_verified', function ($item) {
                if ($item->cnic_verified == 'verified') {
                    return UserCNICStatusEnum::VERIFIED()->toHtml();
                }

                return UserCNICStatusEnum::NOTVERIFIED()->toHtml();
            })
        ...
        return apply_filters(BASE_FILTER_GET_LIST_DATA, $data, $this->repository->getModel())
            ->addColumn('operations', function ($item) {
                ...
                return apply_filters(ACL_FILTER_USER_TABLE_ACTIONS,
                    $action . view('core/acl::users.partials.vendor-actions', ['item' => $item])->render(), $item);
            })
            ->escapeColumns([])
            ->make(true);
    }
    ...
    public function query()
    {
        ...
        $select = [
            ...
            'users.super_user',
            'users.cnic_verified',
            'vendor_companies.company_name as company_name'
        ];

        $query = $model->leftJoin('role_users', 'users.id', '=', 'role_users.user_id')
            ->leftJoin('roles', 'roles.id', '=', 'role_users.role_id')
            ->Join('vendor_companies', 'vendor_companies.user_id', '=', 'users.id')
            ->where('roles.id','=','1')
            ->select($select);
    ...
    public function columns()
    {
        return [
            ...
            'company_name'     => [
                ...
                'title' => trans('core/base::tables.company_name'),
                'class' => 'text-left',
            ],
            'role_name'  => [
                'name'       => 'role_name',
            ...
            'cnic_verified'     => [
                'name'  => 'users.cnic_verified',
                'title' => trans('core/base::tables.cnic_verified'),
                'width' => '100px',
            ],
            'status'     => [
                'name'  => 'users.updated_at',
