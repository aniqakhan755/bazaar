...
class PublicCheckoutController
{
    ...
    )
    {
        ...
        /**
         * Check if products is from same vendor
         */
        $product_ids = array();
        $args = array();
        foreach (Cart::instance('cart')->content() as $cartItem) {
            $product_ids[] = $cartItem->id;
        }
        //$product_count = Product::whereIn('id', $product_ids)->groupBy('user_id')->count();
        $product_user = Product::select('user_id', DB::raw('count(*) as total'))
            ->whereIn('id', $product_ids)
            ->groupBy('user_id')
            ->pluck('total', 'user_id');//->count();
        $product_user_count = sizeof($product_user);
        //dd( $product_user);
        $product_user_keys = $product_user->keys();


        if ($product_user_count > 1) {
            //dd("is multi vendor products");
            //dd(Cart::instance('cart')->content());
            $cartInstances = 'cart_process';
            $multiTokens = array();
            for ($x = 0; $x < $product_user_count; $x++) {
                Cart::instance($cartInstances.$x)->destroy();
            }
            for ($x = 0; $x < $product_user_count; $x++) {
                foreach (Cart::instance('cart')->content() as $cartItem) {
                    $condition = ['id' => $cartItem->id];
                    $condition += ['user_id' => $product_user_keys[$x]];
                    $product = Product::where($condition)->first();
                    //dd($product);
                    if ($product) {
                        Cart::instance($cartInstances . $x)->add(
                            $product->id,
                            $product->name,
                            $cartItem->qty,
                            $cartItem->price,
                            [
                                ...
                                'taxRate' => $cartItem->options->taxRate,
                                'extras' => $cartItem->options->extras,
                            ]);
                    }
                }
            }
            ...
            return view('plugins/ecommerce::orders.multi-checkout', compact(
                'token',
                'shipping',
                'product_user_count',
                'product_user_keys',
...
//                        $sessionData['created_order_address'] = true;
            ...
            if (!empty($sessionData['created_order_address_id'])) {
    ...
    )
    {
        ...
        if ($product_user_count > 1) {
            ...
            //dd(Cart::instance($cartInstances.'1')->content());
            //dd(serialize($multiTokens));
            $tokenMain = $token;
            $orderMain = $this->orderRepository->getFirstBy(compact('token'));


            for ($x = 0; $x < $product_user_count; $x++) {

                $token = md5(Str::random(40));
                array_push($multiTokens, $token);
                //session(['tracked_start_checkout' => $token]);
                $weight = 0;

                foreach (Cart::instance($cartInstances . $x)->content() as $cartItem) {
                    $product = $this->productRepository->findById($cartItem->id);
                    if ($product) {
                        if ($product->weight) {
                            $weight += $product->weight * $cartItem->qty;
                ...
                $promotionDiscountAmount = $handleApplyPromotionsService->executeMulti($token, ($cartInstances . $x));
                $sessionData = OrderHelper::getOrderSessionData($tokenMain);
                OrderHelper::setOrderSessionData($token, $sessionData);
                $sessionData = OrderHelper::getOrderSessionData($token);
                // dd($sessionData);
                ...
                if ($request->has('shipping_method') && !get_shipping_setting('free_ship',
                    ...
                    $shippingData = [
                        ...
                        'weight' => $weight,
                        'order_total' => Cart::instance($cartInstances . $x)->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount,
                    ];
                ...
                $request->merge([
                    'amount' => ($promotionDiscountAmount + $couponDiscountAmount - $shippingAmount) > Cart::instance($cartInstances . $x)
                        ->rawTotal() ? 0 : Cart::instance($cartInstances . $x)
                    ...
                    'tax_amount' => Cart::instance($cartInstances . $x)->rawTax(),
                    'sub_total' => Cart::instance($cartInstances . $x)->rawSubTotal(),
                    'coupon_code' => session()->get('applied_coupon_code'),
                ...
                if ($order) {
                    ...
                    Arr::set($sessionData, 'order_id', $order->id);
            ...
            Session::forget('multiTokens');
            Session::push('multiTokens', $multiTokens);

            if ($orderMain != null) {
                try {
                    $this->orderRepository->deleteBy(['id' => $orderMain->id]);
                    event(new DeletedContentEvent(ORDER_MODULE_SCREEN_NAME, $request, $orderMain));
            ...
            if ($paymentData['error']) {
                return $response
                    ->setError()
                    ->setNextUrl(route('public.checkout.success-multi', $tokenMain))
                    ->setMessage($paymentData['message']);
            }

            //$orderMain = $this->orderRepository->getFirstBy(['token'=>$tokenMain], [], ['address', 'products']);
            //dd($orderMain);
            //$order = $this->orderRepository->findOrFail($order->$id);
    ...
    /**
     * @param string array $multiTokens
     * @param BaseHttpResponse $response
     * @return BaseHttpResponse|Application|Factory|RedirectResponse|View
     */
    public function getCheckoutSuccessMulti($tokenMain, BaseHttpResponse $response)
    {
        ...
        $multiTokens = Session::get('multiTokens')[0];
        //dd($multiTokens);
        $multiOrders = array();
        for ($x = 0; $x < count($multiTokens); $x++) {
            //foreach (Cart::instance('cart')->content() as $cartItem) {
            //array_push($multiTokens,($cartInstances.$x));
            $order = $this->orderRepository->getFirstBy(['token' => $multiTokens[$x]], [], ['address', 'products']);
            array_push($multiOrders, $order);
            //}
        }
        //dd($multiOrders);


        //dd($order. ' : '.session('tracked_start_checkout'));
        /* if ($tokenMain !== session('tracked_start_checkout') || !$order) {
             return $response->setNextUrl(url('/'));
         }*/

        OrderHelper::clearSessions($tokenMain);

        return view('plugins/ecommerce::orders.thank-you-multi', ['multiOrders' => $multiOrders]);
        return $response->setNextUrl(url('/'));
    }
