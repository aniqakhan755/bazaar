        ...
        <div class="row">
            ...
            <div class="col-lg-5 col-md-6 d-none d-md-block right" id="main-checkout-product-info">
                ...
                @for ($x = 0; $x < $product_user_count; $x++)
                    @if (isset($products) && $products)
                        @foreach(Cart::instance($cartInstances. $x)->content() as $key => $cartItem)
                            @php
                                $product = $products->where('id', $cartItem->id)->first();
                            @endphp
                            @if(!empty($product))
                    ...
                    <div class="row price">
                        ...
                        <div class="col-lg-5 col-md-4 col-5">
                            <p class="price-text sub-total-text"> {{ format_price(Cart::instance($cartInstances.$x)->rawSubTotal()) }} </p>
                        </div>
                    </div>
                    @if (session('applied_coupon_code'))
                        <div class="row coupon-information">
                    ...
                    @if (EcommerceHelper::isTaxEnabled())
                        <div class="row shipment">
                            ...
                            <div class="col-lg-5 col-md-4 col-5 float-right">
                                <p class="price-text tax-price-text"> {{ format_price(Cart::instance($cartInstances.$x)->rawTax()) }} </p>
                            </div>
                        </div>
                    @endif
                    <hr/>
                    <div class="row total-price mb-2">
                        ...
                        <div class="col-lg-5 col-md-4 col-5 float-right">
                            <p class="total-text raw-total-text"
                               data-price="{{ Cart::instance($cartInstances.$x)->rawTotal() }}"> {{ ($promotionDiscountAmount + $couponDiscountAmount - $shippingAmount) > Cart::instance($cartInstances.$x)->rawTotal() ? format_price(0) : format_price(Cart::instance($cartInstances.$x)->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount) }} </p>
                        </div>
                    </div>
                @endfor
